...
abstract contract IModuleAuth {
  ...
  event ImageHashUpdated(address account, bytes32 newImageHash);
   ...
   */
  function _isValidImage(address,bytes32) internal virtual view returns (bool) {
    return false;
  }
   ...
   */
  function updateImageHash(address _account, bytes32 _imageHash) external virtual;

  /**
   * @notice Updates the signers configuration of the wallet
   * @param _imageHash New required image hash of the signature
   */
  function _updateImageHash(address _account, bytes32 _imageHash) internal virtual;
}
